...
func Setup(rnd : RandomNumberGenerator, squadsAndRadiiSquared : Array, parent : Control) -> void:
	...
	_region_rid = NavigationServer2D.region_create()
	NavigationServer2D.region_set_map(_region_rid, _map_rid)
	...
	NavigationServer2D.bake_from_source_geometry_data_async(_nav_poly, _source_geom_data, Callable(self, "_on_baking_completed").bind(parent.get_tree()))

func _on_baking_completed(tree : SceneTree) -> void:
	NavigationServer2D.region_set_navigation_polygon(_region_rid, _nav_poly)
	NavigationServer2D.map_set_active(_map_rid, true)
	await tree.physics_frame

func GetPath(p1 : Vector2, p2 : Vector2, _node : Node2D) -> PackedVector2Array:
	if not _map_rid.is_valid():
		return []
	if not _nav_poly:
		return []
	#var node_rid : RID = node.get_world_2d().navigation_map
