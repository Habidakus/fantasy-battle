...
func Update(squad : Squad) -> void:
	...
	_set_size(squad.GetDepthAndWidth())
...
func _set_size(depthAndWidth : Vector2) -> void:
	var depth_mult = depthAndWidth.y / 20.0
	var width_mult = depthAndWidth.x / 20.0
	for p : Polygon2D in _polygons.keys():
		for i in range(_polygons[p].size()):
			p.polygon[i] = Vector2(depth_mult * _polygons[p][i].x, width_mult * _polygons[p][i].y)
